  ...
  <el-table
    ...
    <el-table-column
      ...
      <template slot-scope="scope">
        <el-button @click="edit(scope.row)" type="text" size="medium">modify |</el-button>
        <el-button @click="del(scope.row)" type="text" size="medium">delete |</el-button>
...
export default {
  methods: {
    ...
    del(row){
      const _this = this
      axios.delete('http://localhost:10010/office/offices/delete/' + row.id).then(function (resp){
        console.log(resp)
        if (resp.status === 200){
...
created() {
  const _this = this
  axios.get('http://localhost:10010/office/offices/findall').then(function (resp) {
