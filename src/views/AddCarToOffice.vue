<template>
  <el-form style="width: 70%" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="150px" class="demo-ruleForm">
    ...
    <el-form-item label="availableSchedule" prop="availableSchedule">
      <el-input v-model="ruleForm.availableSchedule" placeholder="for example：2021-06-01:2021-08-01"></el-input>
    </el-form-item>
...
export default {
  data() {
    return {
      ...
      rules: {
        carUid: [
          {required: true, message: 'please enter carUid', trigger: 'blur'},
        ...
        // ],
        registrationNumber: [
          {required: true, message: 'please enter registrationNumber', trigger: 'blur'},
          {min: 1, max: 9, message: '3 to 8 characters in length', trigger: 'blur'}
        ],
        availableSchedule: [
          {required: true, message: 'please enter availableSchedule', trigger: 'blur'},
          {min: 1, max: 21, message: '3 to 8 characters in length', trigger: 'blur'}
        ]
      }
    };
  },
    ...
    submitForm: function (formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios.post('http://localhost:10010/office/offices/' + this.availableCars.officeUid + '/car/' + this.availableCars.carUid, {
  ...
  created() {
    const _this = this
    axios.get('http://localhost:10010/office/offices/' + this.$route.query.officeUid).then(function (resp) {
